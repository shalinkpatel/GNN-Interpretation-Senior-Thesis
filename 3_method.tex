...
As mentioned in section \S\ref{sec:normalizing-flows}, for a fully universal normalizing flow model, more expressive transformations are required than invertible affine transformations. One such method comes from spline coupling normalizing flow layers as described in \cite{dolatabadi_invertible_2020}. In this work, spline transforms are combined with coupling layer transforms.

First, we describe coupling layer transforms. In this category of transforms, the output of a layer consists of two parts. Let $z \in \mathbb{R}^D$ be the data to be transformed. Then we split this data such that $z_1 \in \mathbb{R}^d$ and $z_2 \in \mathbb{R}^{D - d}$. They are then transformed according to
\begin{align*}
	x_1 &= z_1 \\
	x_2 &= g_{\theta(x_1)}(z_2)
\end{align*}
with the final output being $x = [x_1; x_2]$. Plenty of different $g_{\theta(x_1)}$ have been proposed. In this work, the neural linear splines from \cite{dolatabadi_invertible_2020} are used due to their competitive performance compared to quadratic or cubic splines as well as increased computational performance.
\begin{figure}[b]
	\centering
	\includegraphics[width=\textwidth]{images/coupling-flow.png}
	\caption{Different normalizing flow models on a synthetic conditionally distributed dataset. From left to right: ground truth, Glow, i-ResNet, Linear Rational Splines (this method). Taken from \cite{dolatabadi_invertible_2020}}
	\label{fig:coupling-flow}
\end{figure}

Most importantly, these coupling flows are able to capture full conditional distributions and have been used in models such as VAEs and generative models \cite{kobyzev_normalizing_2021} as seen in figure \ref{fig:coupling-flow}. Crucially, this gives an extremely flexible family of distributions that ought to be able to capture the conditional structure that exists within a graph.
